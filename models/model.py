...
def SimpleRNN_dnn_all_states_mout(dropout=0.0, inputs=None):

    inputs = keras.layers.Input(shape=(inputs.shape[1],inputs.shape[2]))
    lstm_out = keras.layers.SimpleRNN(32,dropout=dropout,
                                 return_sequences=True)(inputs) # Todo: make dimension as param
    lstm_out_flatten = keras.layers.Flatten()(lstm_out)
    #lstm_out_flatten = keras.layers.AveragePooling1D(pool_size=inputs.shape[1])(lstm_out)
...
available_models = {"dnn1": dnn1,
                    ...
                    "SimpleRNN_dnn_all_states_mout" : SimpleRNN_dnn_all_states_mout,
                    "ffn": ffn }
